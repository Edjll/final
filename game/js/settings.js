...
nickname.oninput = () => {
	if (!(nickname.value[0] > 'a' && nickname.value[0] < 'z') &&
		!(nickname.value[0] > 'A' && nickname.value[0] < 'Z') && 
		!(nickname.value[0] > 'А' && nickname.value[0] < 'я')) {
...
function sortLocalStorage() {
	if(localStorage.length > 0) {
        ...
        for (i = 0; i < localStorage.length; i++) {
        	if (!isNaN(Number(localStorage.getItem(localStorage.key(i)))) && localStorage.key(i) != 'score') {
